...
> 29 |                 await expect(homePage.logoutNavOption).toHaveText("Log out", {timeout: 10000});
     |                                                        ^ Error: expect(locator).toHaveText(expected) failed
  30 |                 await expect(homePage.nameNavOption).toHaveText(`Welcome ${data.username}`, {timeout: 10000})
  31 |             }
  32 |         })
  33 |     }
  34 | })
