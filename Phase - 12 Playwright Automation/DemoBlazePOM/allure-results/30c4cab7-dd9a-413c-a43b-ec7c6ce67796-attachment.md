...
- Name: Login.test.ts >> Login Tests >> Login test with
...
```ts
  ...
  14 |     
  15 |         
  16 |         test(`Login test with`, async ({page, homePage, loginPage}) => {
  17 |             
  18 |             if(false) {
  19 | 
  20 |                 page.on("dialog", async (alert) => {
  21 |                     expect(alert.message()).toBe("");
  22 |                     await alert.accept();                    
  23 |                 })
  24 |                 await loginPage.login("admin", "admin");
  25 |             }
  26 |             else {
  27 |                 await loginPage.login("admin", "admin");
> 28 |                 await expect(homePage.logoutNavOption).toHaveText("Log out");
     |                                                        ^ Error: expect(locator).toHaveText(expected) failed
  29 |                 await expect(homePage.nameNavOption).toHaveText(`Welcome ${"admin"}`)
  30 |             }
  31 |         })
  32 | 
  33 | })
