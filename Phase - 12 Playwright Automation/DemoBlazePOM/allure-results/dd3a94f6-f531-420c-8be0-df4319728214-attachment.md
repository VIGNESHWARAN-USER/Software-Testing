...
- Name: AddToCart.test.ts >> Add Product To Cart >> Add Samsung Galaxy S6 to cart
- Location: tests\AddToCart.test.ts:5:9

# Error details

```
Error: locator.click: SyntaxError: Failed to execute 'evaluate' on 'Document': The string '//a[text() = Samsung galaxy s6]' is not a valid XPath expression.
...
```ts
  ...
  8  |         this.page = page;
  9  | 
  10 |         this.addToCartButton = page.locator('//a[text()="Add to cart"]');
  11 |     }
  12 | 
  13 |     async addToCart() {
> 14 |         await this.addToCartButton.click();
     |                                    ^ Error: locator.click: Test ended.
  15 |     }
  16 | }
```