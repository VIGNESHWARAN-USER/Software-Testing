...
> 17 |     await expect(page.title()).toContain('STORE');
     |                                ^ TypeError: received is not iterable
  18 | 
  19 |     // const cartPage = await context.newPage();
  20 |     // await cartPage.goto("https://demoblaze.com/cart.html");
  ...
  25 |     const newCartPage = await newContext.newPage();
  26 |     await newCartPage.goto("https://demoblaze.com/cart.html");
  27 |     await expect(page.getByRole('link', { name: 'Log in' })).not.toBeVisible();
  28 | 
  29 | })
```