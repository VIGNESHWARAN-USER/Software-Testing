...
```ts
  ...
  2   | 
  3   | test.describe.only("Login Group", () => {
  ...
  18  | })
  19  | 
  20  | test("Login Test With Soft Assertion", async ({page}) => {
  21  | 
  22  | 
...
> 26  |     await expect.soft(page.getByRole('heading', { name: 'Log out' })).toBeVisible();
  ...
  53  | })
  54  | 
  55  | test.describe.skip("Login Group 2", () => {
  56  |     test("Login Test", async ({page}) => {
  57  | 
  58  | 
  ...
  70  | })
  71  | 
  72  | test("Login Test With Soft Assertion", async ({page}) => {
  73  | 
  74  | 
